...
{
    pthread_mutex_init( &m_MutexHook, NULL );
    CodeSectInit();
    return true;
}
...
{
    HookData_DeleteAll();
    pthread_mutex_destroy( &m_MutexHook );
    return true;
}
...
void* CHookCode::FetchAddress( const char* pczLibName, const char* pczFuncName )
{
    ...
    pHandle = dlopen( pczLibName, RTLD_NOW );
    if(!pHandle)
    {
        printf("[%s] dlopen( %s ), Error=%s \n", __FUNCTION__, pczLibName, dlerror() );
        return NULL;
    }
    
    pFuncAddr = dlsym( pHandle, pczFuncName );
    if(!pFuncAddr)
    {
        dlclose( pHandle );
...
void* CHookCode::FetchMachAddress( const char* pczLibName, const char* pczFuncName )
{
    void* pFuncAddr = NULL;
    
    _dyld_lookup_and_bind_with_hint( pczFuncName, pczLibName, &pFuncAddr, NULL );
    if(!pFuncAddr)
    {
        _dyld_lookup_and_bind( pczFuncName, &pFuncAddr, NULL );
        if(!pFuncAddr)
        {
            return NULL;
        }
...
bool CHookCode::UpdateNextCommand(PHookData pHookInfo)
{
    ...
    vm_protect( mach_task_self(), (vm_address_t)pNextAddr, nNextSize, 0, VM_PROT_READ|VM_PROT_WRITE|VM_PROT_EXECUTE );
    ...
    vm_protect( mach_task_self(), (vm_address_t)pNextAddr, nNextSize, 0, VM_PROT_READ | VM_PROT_EXECUTE );
...
bool CHookCode::UnhookAPI( PVOID pNextAddress )
{
    ...
    //
    nErr = vm_protect( mach_task_self(), (vm_address_t)pFuncAddr, nOldSize, 0, VM_PROT_READ|VM_PROT_WRITE|VM_PROT_EXECUTE );
    if(!nErr)
    {
        memset( pFuncAddr, NOP, nOldSize );
        memcpy( pFuncAddr, pHookInfo->czOldOPCode, nOldSize );
        nErr = vm_protect( mach_task_self(), (vm_address_t)pFuncAddr, nOldSize, 0, VM_PROT_READ | VM_PROT_EXECUTE );
        if(!nErr)
        {
...
bool CHookCode::HookAPI_Int( const char* pczLibName, const char* pczFuncName, void* pHookAddress, void** ppOutNextAddress )
{
    ...
    pFuncAddr = FetchMachAddress( pczLibName, pczFuncName );
    if(!pFuncAddr)
    {
...
PHookData CHookCode::HookData_Search( const char* pczLibName, const char* pczFuncName )
{
    ...
    while(pPos)
    {
        if( !strncasecmp(pPos->pczLibName, pczLibName, strlen(pczLibName)) &&
            !strncasecmp(pPos->pczFuncName, pczFuncName, strlen(pczFuncName)))
        {
            return pPos;
        }
        pPos = pPos->pNext;
...
{
    kern_return_t nError = 0;

    if(!pSectAddr) return false;
    
    nError = vm_protect( mach_task_self(), (vm_address_t)pSectAddr, nCodeSize, 0, VM_PROT_READ|VM_PROT_WRITE|VM_PROT_EXECUTE );
    if(nError)
    {
    ...
    nError = vm_protect( mach_task_self(), (vm_address_t)pSectAddr, nCodeSize, 0, VM_PROT_READ | VM_PROT_EXECUTE );
    if(nError)
    {
...
bool CHookCode::CodeSectMemcpy(void* pSectAddr, void* pMemAddr, int nLength)
{
    ...
    nError = vm_protect( mach_task_self(), (vm_address_t)pSectAddr, nLength, 0, VM_PROT_READ|VM_PROT_WRITE|VM_PROT_EXECUTE );
    if(nError)
    {
    ...
    nError = vm_protect( mach_task_self(), (vm_address_t)pSectAddr, nLength, 0, VM_PROT_READ | VM_PROT_EXECUTE );
    if(nError)
    {
        printf("[%s] pSectAddr=%p, Length=%d vm_protect Failed. 02 \n", __FUNCTION__, pSectAddr, nLength );
        return false;
